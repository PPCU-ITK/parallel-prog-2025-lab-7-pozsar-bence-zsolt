...
int main(){
    ...
    #pragma omp target data \
        map(alloc:rho[0:total_size]) \
        map(alloc:rhou[0:total_size]) \
        map(alloc:rhov[0:total_size]) \
        map(alloc:E[0:total_size]) \
        map(alloc:rho_new[0:total_size]) \
        map(alloc:rhou_new[0:total_size]) \
        map(alloc:rhov_new[0:total_size]) \
        map(alloc:E_new[0:total_size]) \
        map(alloc:solid[0:total_size])
    {
